function varargout = linPF5(Operation,Global,input)
% <problem> <linPF>
...
%--------------------------------------------------------------------------
    ...
    switch Operation
        ...
        case 'value'
            ...
            x=[PopDec(:,1),PopDec(:,2:M-1)*pi*2];
            g      = sum((PopDec(:,M:end)-0.5).^2,2)+abs(sin(pi/2*round(2*x(:,1))));
        ...
        case 'PF'
            ...
            if Global.M==3
                p=ones(1,3)/3;
                [x,y]=meshgrid([0:0.01:0.25, 0.75:0.01:1],linspace(0,1,ceil(h)));
                y=2*y*pi;
